...
// Initialize equipped items when component mounts
onMounted(() => {
  setTimeout(() => {
    const localItems = items;
    const localEquippedItems = [
        localItems.filter((item) => item.category == 'Head')[0],
        localItems.filter((item) => item.category == 'Chest')[0],
        localItems.filter((item) => item.category == 'Legs')[0],
        localItems.filter((item) => item.category == 'MainHand')[0],
    ]
    store.commit('initializeEquippedItems', localEquippedItems);
    loading.value = false;
  }, 3000);
});
...
.loading-spinner {
  width: 60px;
  height: 60px;
  border: 4px solid rgba(163, 53, 238, 0.3);
